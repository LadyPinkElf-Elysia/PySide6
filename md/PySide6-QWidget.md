...
在 PySide6 中，由于底层是 C++ 严格的命名空间，很多枚举需要用“类.子类.值”的方式书写。下表列出了 `QWidget` 章节中出现频率最高的枚举，**供你日后直接查阅**：

### 1. 窗口类型与标志位 (`Qt.WindowType`)
用于 `QWidget.setWindowFlags()`。
| 常用枚举路径 | 中文释义 |
| :--- | :--- |
| `Qt.WindowType.Window` | 标准窗口（默认） |
| `Qt.WindowType.Dialog` | 对话框样式（无最大/最小化按钮） |
| `Qt.WindowType.Popup` | 弹出式菜单窗口（点击其他地方自动消失） |
| `Qt.WindowType.Tool` | 工具窗口（通常不显示在系统任务栏） |
| `Qt.WindowType.FramelessWindowHint` | **无边框窗口**（自定义奇形怪状 UI 必用） |
| `Qt.WindowType.WindowStaysOnTopHint` | **窗口置顶**（永远在最上面） |
| `Qt.WindowType.WindowStaysOnBottomHint` | 窗口置底（永远在最下面） |

### 2. 窗口状态 (`Qt.WindowState`)
用于 `QWidget.setWindowState()`。
| 常用枚举路径 | 中文释义 |
| :--- | :--- |
| `Qt.WindowState.WindowNoState` | 恢复普通状态 |
| `Qt.WindowState.WindowMaximized` | 最大化 |
| `Qt.WindowState.WindowMinimized` | 最小化 |
| `Qt.WindowState.WindowFullScreen` | **全屏** |

### 3. 鼠标光标形状 (`Qt.CursorShape`)
用于 `QWidget.setCursor()`。
| 常用枚举路径 | 中文释义 |
| :--- | :--- |
| `Qt.CursorShape.ArrowCursor` | 默认箭头 |
| `Qt.CursorShape.PointingHandCursor` | 手形（鼠标悬停在按钮、链接上） |
| `Qt.CursorShape.IBeamCursor` | 输入文本时的竖线光标 |
| `Qt.CursorShape.WaitCursor` | 等待转圈（耗时操作时使用） |
| `Qt.CursorShape.ForbiddenCursor` | 禁止标志（🚫） |

### 4. 鼠标按键 (`Qt.MouseButton`)
用于 `QMouseEvent.button()`。
| 常用枚举路径 | 中文释义 |
| :--- | :--- |
| `Qt.MouseButton.LeftButton` | 鼠标左键 |
| `Qt.MouseButton.RightButton` | 鼠标右键 |
| `Qt.MouseButton.MiddleButton` | 鼠标中键（滚轮按下） |

### 5. 键盘按键与修饰符 (`Qt.Key` / `Qt.KeyboardModifier`)
用于 `QKeyEvent.key()` 和 `QKeyEvent.modifiers()`。
| 常用枚举路径 | 中文释义 |
| :--- | :--- |
| `Qt.Key.Key_Escape` | `Esc` 键 |
| `Qt.Key.Key_Return` 或 `Qt.Key.Key_Enter` | 回车键 |
| `Qt.Key.Key_Tab` | `Tab` 制表键 |
| `Qt.Key.Key_A` 到 `Qt.Key.Key_Z` | 字母 A-Z |
| `Qt.Key.Key_0` 到 `Qt.Key.Key_9` | 数字 0-9 |
| `Qt.KeyboardModifier.ControlModifier` | `Ctrl` 键 |
| `Qt.KeyboardModifier.ShiftModifier` | `Shift` 键 |
| `Qt.KeyboardModifier.AltModifier` | `Alt` 键 |

### 6. 消息框标准按钮 (`QMessageBox.StandardButton`)
用于 `QMessageBox.question()` 等静态方法的返回值或参数。
| 常用枚举路径 | 中文释义 |
| :--- | :--- |
...
### 7. 对齐方式 (`Qt.AlignmentFlag`)
用于 `QLabel.setAlignment()` 或 `QPainter.drawText()`。
| 常用枚举路径 | 中文释义 |
| :--- | :--- |
| `Qt.AlignmentFlag.AlignLeft` | 靠左对齐 |
| `Qt.AlignmentFlag.AlignRight` | 靠右对齐 |
| `Qt.AlignmentFlag.AlignHCenter` | 水平居中 |
| `Qt.AlignmentFlag.AlignTop` | 靠上对齐 |
| `Qt.AlignmentFlag.AlignBottom` | 靠下对齐 |
| `Qt.AlignmentFlag.AlignVCenter` | 垂直居中 |
| `Qt.AlignmentFlag.AlignCenter` | **中心对齐**（水平+垂直居中，最常用） |

### 8. 焦点策略 (`Qt.FocusPolicy`)
用于 `QWidget.setFocusPolicy()`。
| 常用枚举路径 | 中文释义 |
| :--- | :--- |
| `Qt.FocusPolicy.StrongFocus` | 鼠标点击和键盘 `Tab` 键都能获取焦点（默认） |
| `Qt.FocusPolicy.ClickFocus` | 只能通过鼠标点击获取焦点 |
| `Qt.FocusPolicy.TabFocus` | 只能通过键盘 `Tab` 键获取焦点 |
| `Qt.FocusPolicy.NoFocus` | **完全不获得焦点**（比如纯装饰性的标签） |
